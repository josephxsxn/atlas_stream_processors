...
data "mongodbatlas_stream_processor" "stream-processor-change-stream" {
  project_id     = var.MONGODB_ATLAS_PROJECT_ID
  instance_name  = "spitest"
  processor_name = "cstftest"
}
...
output "token" {
    value = data.mongodbatlas_stream_processor.stream-processor-change-stream.stats
}
