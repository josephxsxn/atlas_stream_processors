...
locals {
  pipeline = jsondecode(file(var.NEW_PROCESSOR))
  resume_token   = jsondecode(file(var.RESUME_TOKEN))
  mergedConfig = merge(local.pipeline[0]["$source"]["config"], { "startAfter" = jsondecode(local.resume_token["token"]["value"])["changeStreamState"] })
  updated_source = {"$source" = merge(local.pipeline[0]["$source"], { "config" = local.mergedConfig })}
  newProcessor = jsonencode([local.updated_source, local.pipeline[1], local.pipeline[2]])
}

resource "mongodbatlas_stream_processor" "stream-processor-change-stream" {
  project_id     = var.MONGODB_ATLAS_PROJECT_ID
  instance_name  = mongodbatlas_stream_instance.spitest.instance_name
  processor_name = "cstftest"
  pipeline       = local.newProcessor
  state          = "STARTED"
}